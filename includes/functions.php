...
function login()
{
    if (isset($_POST['login'])) {
        ...
        } else if ($password == $data[0]['user_password'] and  $userEmail == $data[0]['email']) {
...
function signUp()
{
    if (isset($_POST['signUp'])) {
        ...
        if (empty($email) or empty($passwd)  or   empty($pincode)  or empty($state) or empty($address) or empty($phn) or empty($fname) or empty($lname)) {
            $_SESSION['message'] = "empty_err";
        ...
        $query = "INSERT INTO user (email ,user_fname ,user_lname , user_phn,user_address,user_state, user_pincode,user_password ) VALUES('$email', '$fname' ,'$lname','$phn','$address' ,'$state' ,'$pincode' ,'$passwd')";
...
{
    $query = "SELECT user_id ,user_fname ,user_lname,user_phn ,email ,user_address,user_state,user_pincode FROM user WHERE user_id=$id";
    $data = query($query);
    return $data;
}
